...
class AirIoImuOdomNode(Node):
    def __init__(self):
        ...
        # === MOD (realign): Cartographer 기반 재초기화 요청 버퍼 ===
    ...
    def odom_callback(self, msg: Odometry):
        ...
        # === MOD (realign): 초기화 후에는, 들어오는 모든 Cartographer /odom으로 '재초기화 요청'
        p = msg.pose.pose.position
        q = msg.pose.pose.orientation
        v = msg.twist.twist.linear
        stamp = msg.header.stamp.sec + msg.header.stamp.nanosec * 1e-9
        ...
        with self._realign_lock:
            self._realign_req = req  # 최신 요청으로 갱신(덮어쓰기)
        # (즉시 재초기화하지 않고 on_timer에서 안전하게 반영)
        # 디버그:
        # self.get_logger().debug(f"Queued realign at t={stamp:.3f}")
    ...
    def on_timer(self):
        ...
        # === MOD (realign): Cartographer 재초기화 요청이 있으면 여기서 반영 ===
        # on_timer에서 반영해야 dt/통합 상태 일관성 보장
        with self._realign_lock:
            req = self._realign_req
            self._realign_req = None
        if req is not None:
            try:
                # Cartographer 타임스탬프 기준으로 재초기화
                device_str = self.get_parameter("device").get_parameter_value().string_value
                self.pp_dev = torch.device(device_str)

                pos0 = torch.tensor(req["pos"], dtype=torch.float64)
                vel0 = torch.tensor(req["vel"], dtype=torch.float64)
                ...
                # 속도 적분기도 같은 초기 pos로 리셋
                self.vel_integ = VelocityIntegrator(
                    pos0, frame=('body' if self.net_vel_is_body else 'world'),
                    method='trapezoid', device='cpu'
                ).double()
                self._last_ego_pos = pos0.numpy().astype(float)

                # 적분 시계 재설정: Cartographer stamp로 맞춤
                ...
                self.get_logger().info(
                    f"Realigned integrators to Cartographer @ t={req['stamp']:.3f} "
                    f"pos={req['pos']}, vel={req['vel']}"
                )
            except Exception as e:
